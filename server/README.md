...
用户可直接在 Android App 中注册用户名和密码，不需要管理员手工分配令牌。每个用户拥有独立的数据密钥、笔记集合和限时登录会话，无法读取其他用户的笔记。

密码采用带随机盐的 Argon2id 摘要，服务端不保存明文密码；App 也不保存密码，只保存 Android Keystore 加密后的随机会话令牌。会话默认 30 天过期，可显式登出，修改密码会撤销全部旧会话。服务端管理数据加密密钥，因此它不是零知识服务。
...
TRUSTED_PROXY_CIDRS=容器网络网关IP/32
```

三个密钥不要重复。`DATA_KEY` 一旦使用就不能更改。`TRUSTED_PROXY_CIDRS` 必须是实际直接连接 API 的 Nginx/Docker 网关 CIDR；未确认时留空，服务端就只使用 `RemoteAddr` 限流并忽略客户端提交的 `X-Real-IP`。
...
首次启动后可用下面的命令查看 API 容器所见的网关，再把该地址以 `/32` 写入 `.env` 并重建 API：

```bash
docker inspect "$(docker compose -f docker-compose.yml -f docker-compose.nginx.yml ps -q api)" \
...
`ALLOW_REGISTRATION=true` 允许任何知道域名的人注册。服务端对每个来源地址限制每分钟 20 次认证请求，并且只信任来自 `TRUSTED_PROXY_CIDRS` 的 `X-Real-IP`。如果只想让朋友在一段时间内注册，可以在大家注册完成后改为：
...
这不会删除旧笔记，但会立即撤销旧恢复令牌及全部已登录设备的会话；随后请使用 `owner + 新密码` 重新登录。
...
先备份 `.env` 和数据卷，再拉取代码。为 Nginx 反代设置精确的 Docker 网关地址，例如：

```dotenv
SESSION_TTL_HOURS=720
TRUSTED_PROXY_CIDRS=172.25.0.1/32
```

旧版 `sessionHashes` 没有签发时间，无法安全计算过期时间，因此升级时会失效；用户需要在 App 中重新输入用户名和密码登录一次。笔记和账户不会被删除。
